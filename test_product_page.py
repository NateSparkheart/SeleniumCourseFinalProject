...
@pytest.mark.parametrize('offer_num', [*range(7), pytest.param(7, marks=pytest.mark.xfail
                         (reason="Некорректное название в сообщении о добавлении в корзину, не чиним, это фича")),
                                       *range(8, 10)])
@pytest.mark.need_review
def test_guest_can_add_product_to_basket(browser, offer_num):
    promo_link = f"http://selenium1py.pythonanywhere.com/catalogue/coders-at-work_207/?promo=offer{offer_num}"
